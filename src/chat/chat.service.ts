...
export class ChatService {
  constructor(
    ...
    @Inject('USERCHATS_REPOSITORY')
    private userChatsRepository: typeof UserChat,
  ...
  async create(
    ...
    try {
      ...
      const chat = await this.chatsRepository.create({
        name: name || `Chat_${userId}`,
        avatar: avatarPath,
        owner: userId,
      });

      const userChat = await this.userChatsRepository.create({
        chatId: chat.id,
        userId: userId,
      });

      JSON.parse(clientsId).forEach(async (item) => {
        await this.userChatsRepository.create({
          chatId: chat.id,
          userId: item,
        });
      });
  ...
  async findMyChats(userId: string): Promise<any> {
    try {
      const alluserChats = await this.userChatsRepository.findAll({
        where: { userId },
      });

      const allUserInAllUserChats = alluserChats.map(async (item) => {
        const chat = await this.userChatsRepository.findAll({
      ...
      const userWithChats = await this.usersRepository.findOne({
        where: { id: userId },
        attributes: { exclude: ['privateKey'] },
        include: [
          {
  ...
  async deleteUsersFromChat(
    deleteUserFromChatDto: DeleteUserFromChatDto,
    chatId: string,
  ): Promise<DeleteUserFromChatType> {
    const { usersId } = deleteUserFromChatDto;
    try {
      usersId.forEach(async (userId) => {
        await this.userChatsRepository.destroy({
          where: {
            userId,
            chatId,
          },
  ...
  async deleteChat(chatId: string): Promise<DeleteChatType> {
    try {
      const chat = await this.chatsRepository.findOne<Chat>({
        where: { id: chatId },
      });
